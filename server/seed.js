...
const seedDatabase = async () => {
  try {
    ...
    const adminData = {
      username: 'admin',
      password: 'admin123', // Remember to change this in a real environment
      role: 'Admin',
    };

    // Use findOneAndUpdate for the user as well to ensure idempotency and handle password hashing
    const existingAdmin = await User.findOne({ username: adminData.username });
    if (!existingAdmin) {
        await User.create(adminData); // create() will trigger the 'pre-save' hook for hashing
